  ...
  .action(async (_appName, options) => {
    ...
    const questions = [
      ...
      },
      {
        type: "confirm",
        name: "credbuildAi",
        message: `CredBuild AI (beta) lets you generate pages using your own components. Learn more: ${ansiColors.cyan}https://credbuild.com/docs/ai/overview${ansiColors.reset}

  Add CredBuild AI to the editor? (Requires a CredBuild Cloud account)`,
        required: true,
        default: true,
      },
    ];
    ...
    const usesCredBuildAi = answers.credbuildAi;

    // Copy template files to the new directory
    const recipeName = `${recipe}${usesCredBuildAi ? "-ai" : ""}`;
    ...
    for (const templateFile of templateFiles) {
      ...
      if (path.extname(filePath) === ".hbs") {
        ...
        data = template({
          ...answers,
          appName,
          credbuildVersion: `^${packageJson.version}`,
        });
      } else {
    ...
    if (!inGitRepo) {
      try {
        ...
        execSync('git commit -m "build(credbuild): generate app"', {
          cwd: appPath,
          stdio: "inherit",
        });
      } catch (error) {
    ...
    if (usesCredBuildAi) {
      console.log(
        `Configure CredBuild AI access: ${ansiColors.cyan}https://cloud.credbuild.com/onboarding/integrate${ansiColors.reset}`
      );
    }
  })
  .parse(process.argv);
